...
from social_media.models import Comment, Follow, Post, Profile
from social_media.serializers import FollowingListSerializer, PostSerializer
...
class AuthenticatedUserPostsApiTests(TestCase):
    ...
    def test_list_user_posts_with_all_fields(self):
        ...
        serializer = PostSerializer(posts, many=True, context={"request": response.wsgi_request})
        ...
        for index, post_data in enumerate(response.data):
            ...
            self.assertEqual(post_data["is_following_author"], expected_is_following_author)
    ...
    def test_filter_user_posts_by_post_date(self):
        ...
        posts = Post.objects.filter(post_date__date=datetime.today().strftime("%Y-%m-%d"))
...
class AuthenticatedPostsApiTests(TestCase):
    ...
    def test_unfollow_post_author(self):
        ...
        Follow.objects.get_or_create(follower=self.user_follower, followee=self.user_post_creator)
    ...
    def test_redirect_to_profile(self):
        ...
        self.assertTrue(
            response_redirect.url.endswith(f"/api/v1/social_media/profile/{self.user_post_creator.username}")
        )


class AuthenticatedFollowingApiTests(TestCase):
    def setUp(self):
        self.user = User.objects.create_user(username="testuser", email="email@email.com", password="12345")
        self.client = APIClient()

        self.followee = User.objects.create_user(username="followee", email="followee@email.com", password="12345")
        self.follower = User.objects.create_user(username="follower", email="follower@email.com", password="12345")
...
class AuthenticatedAddCommentApiTests(TestCase):
    ...
    def test_add_comment_success(self):
        ...
        url = reverse("social_media:add-comment-to-sb", kwargs={"post_id": self.post.id})
        ...
        self.assertEqual(response.data["comments"][0]["comment_text"], "This is a test comment")
        self.assertEqual(response.data["comments"][0]["user"], self.user_commenter.username)

    def test_add_comment_invalid_post(self):
        comment_data = {"comment_text": "This is a test comment"}
        url = reverse("social_media:add-comment-to-sb", kwargs={"post_id": self.post.id + 1})
        response = self.client.post(url, comment_data, format="json")
    ...
    def test_add_comment_missing_comment_text(self):
        ...
        self.assertIn("comment_text", response.data)
        self.assertEqual("This field may not be null.", response.data["comment_text"][0])


class AuthenticatedToggleLikeApiTests(TestCase):
    def setUp(self):
        self.client = APIClient()
        self.user = User.objects.create_user(username="testuser", email="email@gmail.com", password="password")
...
class CommentViewSetTests(TestCase):
    def setUp(self):
        ...
        self.post = Post.objects.create(user=self.post_author, title="Test Post", content="Test content")
    ...
    def test_retrieve_comment(self):
        url = reverse("social_media:user-comments-detail", kwargs={"pk": self.comment.pk})
